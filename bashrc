...
# Helper function to extract the active Git branch name
parse_git_branch() {
  git branch 2>/dev/null | sed -e '/^[^*]/d' -e 's/* \(.*\)/(\1)/'
}

# Two-line configuration featuring a timestamp, path, and Git status
export PS1="\n\[\e[33m\][\t]\[\e[m\] \[\e[36m\]\w\[\e[m\] \[\e[35m\]\$(parse_git_branch)\[\e[m\]\n\$ "
